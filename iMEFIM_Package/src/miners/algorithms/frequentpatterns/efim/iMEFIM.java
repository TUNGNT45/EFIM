...
public class iMEFIM {
	...
	/** If true, build Pset */
	private boolean isBuildPsetList = true;
       ...
       for (int j=0; j< itemsToKeep.size(); j++)
	   {
		...
		if(dbTranMiningLen !=0)
		{
	    	...
	    	if(activateSubtreeUtilityPruning){
	    		...
	    		if (isBuildPsetList){
	    			backtrackingEFIMWithProjectList(dataset.getTransactions(), itemsToKeep, itemsToExplore, 0, PsetList);
	    		}else{
...
//		        }
     ...
     * @throws IOException if error writing to output file
     */
    private void backtrackingEFIMWithProjectList( List<Transaction> transactionsOfP,
...
//		        }
	        ...
	        if (isBuildPsetList){
				/*if(PsetEX == null)
					PsetEX = new ArrayList<ArrayList<Integer>>();*/
	        	for (int i =0 ; i<=newItemCount; i++)
				{
					PsetEX.add(new ArrayList<Integer>());
				}
	        	useUtilityBinArraysToCalculateUpperBoundsWithPsetList(transactionsPe, j, itemsToKeep,PsetEX); 
	        } else{
	        	useUtilityBinArraysToCalculateUpperBounds(transactionsPe, j, itemsToKeep);  
	        }
	        // we now record time for identifying promising items
	    	...
	    	if(activateSubtreeUtilityPruning){
	    		// if sub-tree utility pruning is activated, we consider primary and secondary items
	    		if(isBuildPsetList){
	    			backtrackingEFIMWithProjectList(transactionsPe, newItemsToKeep, newItemsToExplore,prefixLength+1,PsetEX);
	    		}
	    		else{
	    			backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToExplore,prefixLength+1);
	    		}
     ...
     * @param itemsToKeep the list of promising items
     */
    private void useUtilityBinArraysToCalculateUpperBoundsWithPsetList(List<Transaction> transactionsPe, 
